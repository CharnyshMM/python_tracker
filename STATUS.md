...
- календарь из нескольких тематических каледарей, в т.ч. общие для нескольких пользователей
- умный быстрый ввод команд
...
Достоинства:
 ...
 - есть приоритет, есть типы задач
...
**5) Дополнительные характеристики дел**
...
- тэги[]
- категории приоритета (хранятся как число)
- подсписок задач[]
- статус выполнения
- Блок комментариев
...
- комментарий может быть добавлен пользователем, у которого есть разрешение как минимум просматривать задачу.
- комментарии никак не изменяют статус задачи.

**8. Реализация Иерархии**  

- 2 основных составляющих иерархии: _Задача_ и _Списки Задач_.
- _Список Задач_ может существовать только внутри _Задачи_ как спискок ее подзадач.
Иерархия:  

1) _Доска Дел_ - это сущность, похожая на _Задачу_, и имеет дополнительные аттрибуты.  

2) Каждая задача из _Списка Задач_ _Доски дел_ - это обычная задача  

3) У нее могут быть подзадачи, помещающиеся в _Список подзадач_ текущей _Задачи_.  
...
4) Система комментариев к заzдачам и спискам
